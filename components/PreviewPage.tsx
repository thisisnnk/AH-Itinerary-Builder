...
import {
  ...
  Phone, Mail, Car, Heart, CheckCircle2, XCircle, CreditCard, Star, Clock, Globe, Download, Loader2, MessageSquare, Compass, Headset
} from 'lucide-react';
...
const PreviewPage: React.FC<Props> = ({ data, onEdit, onBack }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      // 1. Create a dedicated wrapper to enforce desktop context
      // We place this off-screen but valid in the DOM
      ...
      // Override clone styles to fill the wrapper
      Object.assign(clone.style, {
        width: '100%',
        height: 'auto',
        margin: '0',
        padding: '0',
        transform: 'none', // Reset main container transform if any
        position: 'static'
      });

      // 3. CSS Sanitation: Disable animations/transitions via injected style
      ...
      style.textContent = `
        * {
        ...
        /* Fallback for backdrop-blur which html2canvas largely incorrectly renders */
        [class*="backdrop-blur"] {
          backdrop-filter: none !important;
          background-color: rgba(255, 255, 255, 0.15) !important; /* Higher opacity fallback */
        }
      `;
      clone.appendChild(style);

      // 4. Manual cleanups for specific heavy elements if needed
      // (Optional: remove existing high-res background patterns if they cause noise? Keeping for now)

      wrapper.appendChild(clone);

      // 5. Allow DOM to settle and images/fonts to render
      await new Promise(resolve => setTimeout(resolve, 2000));

      const canvas = await html2canvas(clone, {
        useCORS: true,
        scale: 2, // High DPI
        backgroundColor: '#01003d', // Match theme bg
        width: 1200, // Match wrapper
        windowWidth: 1200,
        logging: true, // Enable for debug if needed
        onclone: (clonedDoc, clonedEl) => {
          // Double-check specific adjustments in the cloned context
          // e.g., ensure svg patterns are visible
          const patterns = clonedEl.querySelectorAll('.topography-pattern');
          patterns.forEach(p => (p as HTMLElement).style.opacity = '0.1'); // Boost pattern visibility slightly
        }
      });
          ...
          <section className="pt-0 pb-12 space-y-12 border-b border-white/5 relative z-10 flex flex-col items-center text-center">
            ...
            <div className="p-12 bg-white/[0.04] backdrop-blur-md border border-white/10 rounded-[40px] max-w-3xl w-full mx-auto relative group overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col items-center gap-8">
                <div className="w-24 h-24 rounded-full bg-[#FECC00]/10 flex items-center justify-center text-[#FECC00] border border-[#FECC00]/20 shadow-[0_0_30px_rgba(254,204,0,0.1)]">
                  <Headset size={48} />
                </div>
